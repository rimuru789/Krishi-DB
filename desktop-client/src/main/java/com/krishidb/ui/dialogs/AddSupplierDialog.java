...
public class AddSupplierDialog extends JDialog {
    ...
    public AddSupplierDialog() {
        setTitle(I18n.get("supplier.dialog.add_title"));
        setSize(480, 460);
    ...
    private JPanel createHeader() {
        ...
        JLabel title = new JLabel(I18n.get("supplier.dialog.add_header"));
        title.setFont(new Font("SansSerif", Font.BOLD, 22));
        title.setForeground(new Color(15, 23, 42));

        JLabel subtitle = new JLabel(I18n.get("supplier.dialog.add_subheader"));
    ...
    private JPanel createButtons() {
        ...
        JButton save = new JButton(I18n.get("supplier.btn.save"));
    ...
    private void saveSupplier() {
        ...
        if (name.length() < 2) {
            JOptionPane.showMessageDialog(this, I18n.get("supplier.msg.val_name_required"), I18n.get("supplier.dialog.add_title"), JOptionPane.WARNING_MESSAGE);
            nameField.requestFocus();
            return;
        }

        if (!phone.matches("^[6-9]\\d{9}$")) {
            JOptionPane.showMessageDialog(this, I18n.get("supplier.msg.val_phone_invalid"), I18n.get("supplier.dialog.add_title"), JOptionPane.WARNING_MESSAGE);
            phoneField.requestFocus();
            return;
        }

        Supplier supplier = new Supplier(name, phone, village);
        boolean success = supplierDAO.addSupplier(supplier);

        if (success) {
            JOptionPane.showMessageDialog(this, I18n.get("supplier.msg.add_success"), I18n.get("supplier.dialog.add_title"), JOptionPane.INFORMATION_MESSAGE);
            dispose();
        } else {
